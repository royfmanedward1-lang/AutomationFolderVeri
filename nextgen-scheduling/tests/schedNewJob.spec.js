...
test.describe('Search and select required fields to schedule a new job', () =>{

    test('Search and select required fields to schedule a new job for location type Veritext', async({page}) => {
        const newTab = new PageManager(page)
        const clientPage = newTab.Client
        const jobPage = newTab.Job
        const i = 0
        const locationType = 'Veritext'
        ...
        await test.step('Select Litigation type', async() => {
            await jobPage.selectLocationType(locationType, 'locationClientName')
        }) 
        await test.step('Verify Time zone is populated', async() =>{
            await jobPage.verifyTimeZoneIsSelected()
        })
