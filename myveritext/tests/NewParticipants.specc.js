...
test.describe('Create New Proceeding with New Participants', () => {
  ...
  test('Schedule new proceeding with new attorney and contact', async () => {
    ...
    await test.step('Add new attorney', async () => {
      ...
      await participantsPage.selectAttyLawyer(`${newAttorneyData.firstName} ${newAttorneyData.lastName}`);
      console.log("Attorney selected from dropdown");
    });
    
    await test.step('Add new point of contact', async () => {
      console.log("new contact started");
      await participantsPage.clickAddContactButton();
      ...
      }, false);  // false for contact
      await participantsPage.clickAddContactConfirmButton();
      await participantsPage.selectPointOfContact(`${newContactData.firstName} ${newContactData.lastName}`);
    ...
    await test.step('Verify success and participants', async () => {
      ...
      const attorneyInfo = await pages.jobCard.getAttorneyInfo();
      expect(attorneyInfo.name).toBe(`${newAttorneyData.firstName} ${newAttorneyData.lastName}`);
      expect(attorneyInfo.email).toBe(newAttorneyData.email);

      const contactInfo = await pages.jobCard.getContactInfo();
      expect(contactInfo.name).toBe(`${newContactData.firstName} ${newContactData.lastName}`);
      expect(contactInfo.email).toBe(newContactData.email);
    });
  });
