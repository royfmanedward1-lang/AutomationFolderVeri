...
test.describe("Cancel Scheduled Jobs", () => {
  ...
  test("Successful Cancel future scheduled job", async () => {
    await test.step("And try to cancel the job scheduled", async () => {
      ...
      await jobDetails.clickOptionsButton();
      await jobDetails.clickCancel();
      await jobDetails.clickYesButton();
    });
    
    await test.step("Then the job scheduled won't be cancelled", async () => {
      expect(jobCardPage.getSuccessfulCancelAlert()).toBeTruthy();
      const status = await jobCardPage.verifyCancelledStatus();
      expect(status).toContain(testData.jobCardDetails.cancelledStatus);
      await page.reload();
    });
  });
    ...
    test("Unsuccessful Cancel future scheduled job", async () => {
      await test.step("And try to cancel the job scheduled", async () => {
        ...
        await jobDetails.clickOptionsButton();
        await jobDetails.clickCancel();
        await jobDetails.clickNoButton();
