...
test.describe("Job Creation In-Person Proceeding", () => {
  ...
  const commonJobCreationSteps = async (locationOption) => {
    ...
    await test.step("And selects the proceeding date and time", async () => {
      ...
      await dateAndTimePage.selectDate(); //Using date selected by default in the date picker. Will be refining this to select a random date on a revise version.
