...
const jobsByFiltersQuery = require('../../GDS/querys/jobsByFilters');
...
// Test
test('Get Jobs for UserId', async ({ request, baseURL }) => {
const userId = parseInt(process.env.USERID, 10);
const depoStartDateMin = process.env.depoStartDateMin;
const depoStartDateMax = process.env.depoStartDateMax;
  ...
  const userResponse = await request.post(baseURL + '/graphql', {
      ...
      data: {
          operationName: "jobsByFilters",
          query: jobsByFiltersQuery,
          variables: {
            "filters": {
              "enablePaging": false,
              "depoStartDateMin": depoStartDateMin,
              "depoStartDateMax": depoStartDateMax,
              "permittedUserIds": [
                userId
              ]
            }
