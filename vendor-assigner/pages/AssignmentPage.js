...
export class AssignmentPage {
    ...
    async changeVendorStatus(jobId, status) {
        const everyPartnerStatusAvailable = await this.page.locator(`//*[@data-id="${jobId}"]`).getByLabel("Select Partner Status").count()
        const randomNum = Math.floor(Math.random() * everyPartnerStatusAvailable) - 1;
        await this.page.locator(`//*[@data-id="${jobId}"]`).getByLabel("Select Partner Status").nth(randomNum).click();
